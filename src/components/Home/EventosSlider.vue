<template>
    <div class="relative w-full mx-auto my-10">
      ...
      <div class="absolute z-20 w-full h-full flex items-center justify-between px-4">
        <button 
          @click="prevSlide" 
          class="bg-black/30 hover:bg-black/60 p-3 rounded-full text-white"
      ...
      <div class="relative w-full h-[500px] overflow-hidden">
        ...
        >
          <div
            ...
            :key="index"
            class="flex-shrink-0 px-2 h-full"
          ...
          >
            <div class="h-full flex flex-col rounded-lg shadow-lg overflow-hidden bg-white">
              ...
              <div class="p-4 bg-white flex-grow">
                <h2 class="font-bold text-xl text-gray-800 mb-2">{{ evento.title }}</h2>
                <p class="text-gray-600 text-sm">{{ evento.description }}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="flex justify-center mt-6 gap-3">
        <button
          ...
          :class="currentIndex === i-1 ? 'bg-primary-500 w-4' : 'bg-gray-300'"
